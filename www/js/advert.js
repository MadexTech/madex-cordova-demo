class Advert extends HTMLElement {
  ...
  render() {
    ...
    loadAdBtn.onClick = () => {
      if (window.Madex.canLoadAd(adType)) {
        this.addLog("Ad start to load.");
        window.Madex.loadAd(adType, placementName);
      } else {
    ...
    showAdBtn.onClick = () => {
      if (window.Madex.isAdLoaded(adType)) {
        window.Madex.showAd(adType, placementName);
      } else {
    ...
    destroyAdBtn.onClick = () => {
      window.Madex.destroyAd(adType, placementName);
